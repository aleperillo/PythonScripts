...
try:
    ...
    sql_server_cursor.execute('TRUNCATE TABLE Cetelem.logUser')
    ...
    for document in mongo_collection.find():
        ...
        sql_query = """
            INSERT INTO Cetelem.logUser (_id, ciudad_localizacion, control1, direccion_ip, extra, fechaLocal, fechaServidor, horaLocal, horaServidor, idUsuario, pagina)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """
...
# Cerrar conexiones
    ...
    logging.info(f'Ejecución del archivo load_Cetelem_logUser.py de forma exitosa: {datetime.datetime.now()}')
